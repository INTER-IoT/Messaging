 ...
 */
public class MiddlewarePayloadFunctions extends MessagePayloadFunctions {

    /*
 Properties:

 */
    private static EntityID entityType = new EntityID("http://inter-iot.eu/GOIoTP#Middleware");

    public static EntityID getEntityType() {
        return entityType;
    }

    public void createMiddleware(MessagePayload payload, EntityID MiddlewareID) {
        payload.getJenaModel().add(MiddlewareID.getJenaResource(),
                RDF.type,
                entityType.getJenaResource());
    }

    public Set<EntityID> getMiddlewares(MessagePayload payload) {
